...
http://192.168.xxx.xxx:81/stream   //Nhận video phát trực tuyến      Nhận video trực tuyến <img src="http://192.168.xxx.xxx:81/stream">
http://192.168.xxx.xxx/capture     //Lấy hình         Lấy hình <img src="http://192.168.xxx.xxx/capture">
http://192.168.xxx.xxx/status      //Nhận giá trị trạng thái hình ảnh

Định dạng đặt hàng tùy chỉnh  http://192.168.xxx.xxx/control?cmd=P1;P2;P3;P4;P5;P6;P7;P8;P9
...
http://192.168.xxx.xxx/control?var=xxx&val=xxx              //Định dạng hướng dẫn chính thức
http://192.168.xxx.xxx/control?var=flash&val=value          //flash value= 0~255
http://192.168.xxx.xxx/control?var=framesize&val=value      //Khung hình value = 10->UXGA(1600x1200), 9->SXGA(1280x1024), 8->XGA(1024x768) ,7->SVGA(800x600), 6->VGA(640x480), 5 selected=selected->CIF(400x296), 4->QVGA(320x240), 3->HQVGA(240x176), 0->QQVGA(160x120), 11->QXGA(2048x1564 for OV3660)
http://192.168.xxx.xxx/control?var=quality&val=value        //Chất lượng hình ảnh value = 10 ~ 63
http://192.168.xxx.xxx/control?var=brightness&val=value     //độ sáng value = -2 ~ 2
http://192.168.xxx.xxx/control?var=contrast&val=value       //tương phản value = -2 ~ 2
*/

//Nhập mật khẩu tài khoản kết nối WIFI
...
//Số lượng hình ảnh đã đăng ký về khuôn mặt của cùng một người
#define ENROLL_CONFIRM_TIMES 5

//Hình ảnh CIF độ phân giải có thể được lấy bằng nút chụp ảnh trên trang web và được lưu trong thẻ SD http://192.168.xxx.xxx/capture  (FRAMESIZE_CIF)
// String filepath[5] = {"/Face/1.jpg", "/Face/2.jpg", "/Face/3.jpg", "/Face/4.jpg", "/Face/5.jpg"};  //1.jpg, 2.jpg, ...., 35.jpg
int image_width = 400;  
...
String recognize_face_matched_name[1] = {"Huy"};
...
#include "fr_forward.h"          //chức năng nhận dạng khuôn mặt
#include "FS.h"                  //chức năng hệ thống tệp
#include "SD_MMC.h"              //Chức năng truy cập thẻ SD
#include "esp_http_server.h"     //Chức năng máy chủ HTTP
#include "esp_timer.h"           //chức năng hẹn giờ
#include "time.h"
String Feedback="";   // trả lại tin nhắn của khách hàng
//  giá trị tham số lệnh
String Command="",cmd="",P1="",P2="",P3="",P4="",P5="",P6="",P7="",P8="",P9="";
//Giá trị trạng thái tháo gỡ hướng dẫn 
...
// Đặt nhận diện khuôn mặt, khung nhận dạng, các biến màu văn bản
...
// Cài đặt tiêu đề trang web chuyển hình ảnh
...
static int8_t detection_enabled = 1;     //trạng thái phát hiện khuôn mặt 0 or 1
...
#define MQTT_USER "phuy2712"
#define MQTT_PASSWORD "phuy2712"
#define MQTT_LDP_TOPIC "Log-in"
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.setDebugOutput(true);  //Bật debug
  Serial.println();
  ...
  //Kích thước mặc định của khung video có thể tùy chỉnh (kích thước độ phân giải)
  ...
  s->set_framesize(s, FRAMESIZE_QVGA);    //Độ phân giải UXGA(1600x1200), SXGA(1280x1024), XGA(1024x768), SVGA(800x600), VGA(640x480), CIF(400x296), QVGA(320x240), HQVGA(240x176), QQVGA(160x120), QXGA(2048x1564 for OV3660)

  //s->set_vflip(s, 1);  //lật dọc
  //s->set_hmirror(s, 1);  //Gương ngang
  ...
  //Chỉ định IP tĩnh của máy khách
  ...
  for (int i=0;i<2;i++) {
    ...
    if (WiFi.status() == WL_CONNECTED) {    //Nếu kết nối thành công
      WiFi.softAP((WiFi.localIP().toString()+"_"+(String)apssid).c_str(), appassword);   //Đặt SSID để hiển thị IP máy khách         
      Serial.println("");
...
void FaceMatched(int faceid) {  //Nhận dạng khuôn mặt đã đăng ký và thực hiện điều khiển lệnh
  struct tm timeinfo;
  ...
  char time[100];
  strftime(time,100, "%A, %B %d %Y %H:%M:%S", &timeinfo );
  String mess;
  if(faceid == -1){
      mess = "Instruder";
  }
  else{
    mess = recognize_face_matched_name[faceid];
  }
  mess  =  mess + " logged in at " + time;
  client.publish(MQTT_LDP_TOPIC,mess.c_str()); 
  if (faceid==0) {  
  } 
  else if (faceid==1) { 
  } 
...
void FaceNoMatched() {  //Nhận dạng khuôn mặt của người lạ và thực hiện điều khiển lệnh
...
void encrollImageSD() {
  ...
  for(const String name: recognize_face_matched_name){
    for(int i = 1; i <= 5; i++){
      filepath.push_back("/Face/" + name + "-" + i + ".jpg");
    }
  }
  for (const String file_name: filepath) {
    File file = fs.open(file_name.c_str());
    Serial.println("detect file: "+ file_name);
    if(!file){
      ...
      if (!image_matrix) {
          ...
          fmt2rgb888((uint8_t*)buf, file.size(), PIXFORMAT_JPEG, image_matrix->item);  //Chuyển đổi định dạng hình ảnh sang định dạng RGB
          box_array_t *net_boxes = face_detect(image_matrix, &mtmn_config);  //Phát hiện khuôn mặt
          if (net_boxes){
            Serial.println("faces = " + String(net_boxes->len));  //Số khuôn mặt được phát hiện
            Serial.println();
            for (int i = 0; i < net_boxes->len; i++){  //Liệt kê vị trí và kích thước khuôn mặt
            ...
            Serial.println("No Face");    //Không nhận diện được khuôn mặt
            Serial.println();
          }
...
//Xử lý vị trí văn bản nhận dạng khuôn mặt đầu ra hình ảnh
...
static int run_face_recognition(dl_matrix3du_t *image_matrix, box_array_t *net_boxes){
    ...
    if (align_face(net_boxes, image_matrix, aligned_face) == ESP_OK){
        if (is_enrolling == 1){  //khuôn mặt đã đăng ký
            int8_t left_sample_face = enroll_face(&id_list, aligned_face);
            ...
            if (matched_id >= 0) {  //Nếu được công nhận là một khuôn mặt đã đăng ký
                ...
                FaceMatched(matched_id);  //Nhận dạng khuôn mặt đã đăng ký và thực hiện điều khiển lệnh
            } else {  //Nếu nó được nhận dạng là một khuôn mặt chưa đăng ký
                Serial.println("No Match Found");
                rgb_print(image_matrix, FACE_COLOR_RED, "Intruder Alert!");
                matched_id = -1;
                FaceMatched(-1);  //Nhận dạng khuôn mặt của người lạ và thực hiện điều khiển lệnh
            }
        }
    } else {  //Nếu một khuôn mặt được phát hiện, nhưng không thể được nhận dạng
                    ...
                    if(!fmt2rgb888(fb->buf, fb->len, fb->format, image_matrix->item)){
                        ...
                        if (net_boxes || fb->format != PIXFORMAT_JPEG){
                            if(net_boxes){
                                ...
                                draw_face_boxes(image_matrix, net_boxes, face_id);  //vẽ hộp mặt
                                dl_lib_free(net_boxes->score);
                                dl_lib_free(net_boxes->box);
                                dl_lib_free(net_boxes->landmark);
                                dl_lib_free(net_boxes);                                
...
// Hiển thị trạng thái thông số video (phải trả về định dạng json)
...
// Giải nén chuỗi lệnh thành một biến
